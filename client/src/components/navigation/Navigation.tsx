                ...
                <div className='max-w-screen-xl mx-auto flex justify-between items-center'>
                    ...
                    <div className='font-bold flex space-x-10 items-center'>
                        {menuItems.map((item, i) => (
                            <p onClick={() => redirectTo(i)} key={i} className={`text-xl cursor-pointer transform transition-all hover:scale-105 hover:text-${color} ${(item.name !== '/' && location.pathname.includes(item.name) == true) ? 'text-blue-500 border-b-4 border-blue-500' : item.name == location.pathname ? 'text-blue-500 border-b-4 border-blue-500' : 'text-white'}`}>{item.label}</p>
                        ))}
                    </div>
                    <div className=''>
